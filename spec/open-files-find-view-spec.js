...
describe('OpenFilesFindView', () => {
  ...
  let activationPromise, searchPromise, editor, editorElement, findView,
      openFilesFindView, workspaceElement;
  ...
  describe("finding", () => {
    ...
    describe("when the find string contains an escaped char", () => {
      beforeEach(async () => {
        ...
        const filePath = path.join(projectPath, "tabs.txt");
    ...
    describe("description label", () => {
      ...
      it("indicates that it's searching, then shows the results", async () => {
        ...
        expect(openFilesFindView.refs.descriptionLabel.textContent).toContain('13 results found in 2 files');
        atom.commands.dispatch(openFilesFindView.element, 'core:confirm');
        expect(openFilesFindView.refs.descriptionLabel.textContent).toContain('13 results found in 2 files');
      });
    ...
    describe("when core:confirm is triggered", () => {
      ...
      describe("when results exist", () => {
        ...
        it("only searches paths matching text in the path filter", async () => {
          spyOn(PathSearcher, 'searchPaths').andCallFake(async () => {});
          openFilesFindView.pathsEditor.setText('*.js');

          atom.commands.dispatch(openFilesFindView.element, 'core:confirm');
          await searchPromise;

          expect(PathSearcher.searchPaths.argsForCall[0][1].paths).toEqual(['*.js']);
        });
    ...
    describe("history", () => {
      beforeEach(() => {
        ...
        openFilesFindView.pathsEditor.setText('abc');
        ...
        openFilesFindView.pathsEditor.setText('def');
        atom.commands.dispatch(openFilesFindView.findEditor.getElement(), 'core:confirm');
      });
      ...
      it("can navigate the entire history stack", () => {
        ...
        expect(openFilesFindView.pathsEditor.getText()).toEqual('def');

        atom.commands.dispatch(openFilesFindView.pathsEditor.element, 'core:move-up');
        expect(openFilesFindView.pathsEditor.getText()).toEqual('abc');

        atom.commands.dispatch(openFilesFindView.pathsEditor.element, 'core:move-down');
        expect(openFilesFindView.pathsEditor.getText()).toEqual('def');

        atom.commands.dispatch(openFilesFindView.pathsEditor.element, 'core:move-down');
        expect(openFilesFindView.pathsEditor.getText()).toEqual('');
    ...
    describe("when the replace button is pressed", () => {
      ...
      describe("when there are search results after a replace", () => {
        it("runs the search after the replace", async () => {
          ...
          expect(openFilesFindView.refs.descriptionLabel.textContent).toContain('Replaced items with items-123 13 times in 2 files');
          ...
          expect(openFilesFindView.refs.descriptionLabel.textContent).toContain("13 results found in 2 files for items");
        })
      });
    });
    ...
    describe("when the open-files-find:replace-all is triggered", () => {
      ...
      describe("when a search with results has been run", () => {
        ...
        it("replaces all the matches and updates the results view", async () => {
          ...
          expect(openFilesFindView.refs.descriptionLabel.textContent).toContain("Replaced items with sunshine 13 times in 2 files");
  ...
  describe("panel focus", () => {
    ...
    it("moves focus between editors with find-and-replace:focus-next", () => {
      ...
      expect(openFilesFindView.pathsEditor.element).toHaveFocus()
