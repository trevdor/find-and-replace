...
class ProjectFindView {
  constructor(model, {findBuffer, replaceBuffer, pathsBuffer, findHistoryCycler, replaceHistoryCycler, pathsHistoryCycler}) {
    ...
    this.pathsPlaceholderText = 'File/directory pattern. eg. `src` to search in the "src" directory or `*.js` to search all JavaScript files';
    this.savedPathsPattern = '';
        ...
        $.header({className: 'header'},
          ...
          $.span({className: 'header-item options-label pull-right'},
            ...
            $.span({className: 'btn-group btn-toggle btn-group-options'},
              ...
              $.button({ref: 'openFilesOptionButton', className: 'btn option-open-files'},
                $.svg({className: "icon", innerHTML:`<use xlink:href="#find-and-replace-icon-files" />`})
              ),
        ...
        $.section({className: 'input-block paths-container'},
          $.div({className: 'input-block-item editor-container'},
            etch.dom(TextEditor, {
              ref: 'pathsEditor',
              mini: true,
              placeholderText: this.pathsPlaceholderText,
  ...
  get openFilePaths() { return atom.workspace.getTextEditors().map(e => this.relPathFromTextEditor(e)) }
  ...
  didShow() {
    ...
    this.updateOpenFilesOptionButtonEnablement();
    this.updatePathsEditor(this.model.getFindOptions().openFiles);
    ...
    this.tooltipSubscriptions = new CompositeDisposable(
      ...
      atom.tooltips.add(this.refs.openFilesOptionButton, {
        title: "Open Files",
  ...
  handleEvents() {
    ...
    this.subscriptions.add(atom.commands.add(this.element, {
      ...
      'project-find:toggle-open-files-option': () => this.toggleOpenFilesOption(),
    ...
    this.refs.openFilesOptionButton.addEventListener('click', () => this.toggleOpenFilesOption());
    ...
    this.refs.pathsEditor.editorElement.addEventListener('click', (e) => this.handlePathsEditorClick(e));
  ...
  search(options) {
    ...
    if (this.model.getFindOptions().openFiles) {
      const paths = this.openFilePaths;
  ...
  relPathFromTextEditor(editor) {
    let [rootPath, relPath] = atom.project.relativizePath(editor.getPath());
  ...
  findInCurrentlySelectedDirectory(selectedElement) {
    // TODO: update find in open files button and enable paths editor
  ...
  updateOpenFilesOptionButtonEnablement() {
    if (this.refs.openFilesOptionButton && atom.workspace.getTextEditors().length === 0) {
      this.refs.openFilesOptionButton.classList.add('disabled');
      this.updateOptionViews();
    }
  }
  ...
  updateOptionsLabel() {
    ...
    if (this.model.getFindOptions().openFiles && !this.refs.openFilesOptionButton.classList.contains('disabled')) {
      label.push('Within Open Files');
    }
  ...
  updatePathsEditor(openFilesOptionWillBeEnabled) {
    if (openFilesOptionWillBeEnabled) {
      this.savedPathsPattern = this.pathsEditor.getText();
      this.pathsEditor.setPlaceholderText('Find and replace only within open files');
    } else {
      this.pathsEditor.setPlaceholderText(this.savedPathsPattern || this.pathsPlaceholderText);
    }
  }
  ...
  updateOptionButtons() {
    ...
    this.setOptionButtonState(this.refs.openFilesOptionButton, this.model.getFindOptions().openFiles);
  ...
  toggleOpenFilesOption() {
    const openFilesOptionEnabled = !this.model.getFindOptions().openFiles;
    this.updatePathsEditor(openFilesOptionEnabled);
    this.search({onlyRunIfActive: true, openFiles: openFilesOptionEnabled});
  }
};
