...
class OpenFilesFindView {
  ...
  getOpenFilePaths() {
    const arr = atom.workspace.getTextEditors().map(editor => {
  ...
  search(options) {
    ...
    let pathsPattern = this.getOpenFilePaths().join(',');
  ...
  replaceAll() {
    ...
    return this.showResultPane().then(() => {
      const pathsPattern = this.getOpenFilePaths().join(',');
  ...
  clearMessages() {
    ...
    this.setInfoMessage('Find in Open Files <span class="subtle-info-message">Close this panel with the <span class="highlight">esc</span> key</span>');
    this.refs.replacmentInfoBlock.style.display = 'none';
  }
